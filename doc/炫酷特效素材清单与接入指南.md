...
| **`NuclearDetonation`** | **`WFX_Explosion Massive`** | **`CFXR4 Explosion Orange (HDR)`**（自动绑定，scale ≈ 2.85） |
| **`NuclearStrikeWarning`** | **`WFX_Explosion Small`** | **`CFXR4 Sparks Explosion`**（自动绑定，scale ≈ 1.45） |
...
`ApocalypseKingVfxPrefabBinder` 会按 prefab 名称关键字写入 `Assets/Resources/Battle/Effects/Effect_*.asset` 的 **Prefab** 与 **prefabScaleMultiplier**。

**核武**（`NuclearDetonation` / `NuclearStrikeWarning`）优先 **War FX `WFX_Nuke`** 与 **CFXR `CFXR Explosion Smoke 2 Solo (HDR)`**（免费包内实际存在的 Prefab）。未导入商店包时会先绑 Kenney 过渡；导入后执行 `.\tools\bind-vfx-prefabs.ps1` 或菜单 **Bind Store VFX Prefabs**。**Validate Nuclear VFX Binding** 可验收。
