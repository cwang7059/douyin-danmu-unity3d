...
```json
{
  ...
  "running": true,
  "port": 8765,
  "pendingHttpMessages": 0,
  "receivedHttpMessages": 0,
  "acceptedHttpMessages": 0,
  "droppedHttpMessages": 0,
  "pendingCommands": 0,
  "acceptedCommands": 0,
  "droppedCommands": 0,
  "lastAcceptedCommand": "",
...
HTTP 请求成功返回 `202`，表示事件已经进入 Unity 主线程待处理队列；最终是否被命令队列接受，可通过 `/health` 或 `/stats` 里的 `acceptedCommands` / `droppedCommands` / `lastDropReason` 查看。
...
组件：`DanmuWebSocketGateway`（与 HTTP 网关共用 `DanmuLiveIngress` 解析逻辑）
...
外部服务把与上文相同的 JSON 文本帧推送到该地址即可，内部会调用 `DanmuCommandQueue.EnqueueRawMessage` 或结构化 `Enqueue`。

测试工具见 `tools/live-danmu-bridge/README.md`。
